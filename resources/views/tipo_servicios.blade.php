    ...
    <div class="min-h-screen bg-gray-100">
        ...
        <div class="flex min-h-screen bg-gray-100">
            <aside class="w-35 bg-gray-500 shadow-md px-5 py-2 space-y-4">
                <nav class="space-y-2">
                    <a href="{{ route('tipo_servicios.create') }}" class="block px-5 py-2 rounded hover:bg-gray-700 text-white font-medium">Crear nuevo servicio</a>
                </nav>
            </aside>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $tipoServicio->porcentaje }}%
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    $ {{ number_format($tipoServicio->precio, 0) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex items-center justify-end gap-4">
                                    <a href="{{ route('tipo_servicios.edit', $tipoServicio->id) }}" class="text-indigo-600 hover:text-indigo-900">Editar</a>
                                    <form class="inline-block" action="{{ route('tipo_servicios.destroy', $tipoServicio->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas eliminar este tipo de servicio?')">
                                        @csrf
                                        @method('DELETE')
