...
<body class="bg-gray-100 flex items-center justify-center min-h-screen p-4">
    ...
    <div class="bg-white p-8 rounded-lg shadow-xl border-4 border-gray-400 w-11/12 md:w-3/4 lg:w-1/2">
        ...
        <div class="flex flex-col items-center justify-center mb-6">
            ...
            <img src="/img/Logo_todo_estilo.png" alt="Logo" class="h-24 mb-4"> {{-- Ajusté la altura del logo a h-24 para que coincida con los formularios anteriores --}}
            <h1 class="text-3xl italic font-serif text-gray-800 text-center">Registrar ingreso</h1>
        </div>

        <form action="{{ route('registro_ingresos.store') }}" method="POST" class="space-y-4"> {{-- Añadí space-y-4 para un mejor espaciado entre elementos del formulario --}}
            @if ($errors->any())
            ...
            <table class="w-full"> {{-- Aseguramos que la tabla ocupe todo el ancho disponible en el contenedor --}}
                <tr>
                    ...
                    <td>
                        <input
                            ...
                            value="{{ old('fecha') }}" {{-- Usamos old() para mantener el valor si hay un error de validación --}}
                            required
                        >
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select name="entrada" id="entrada" class="w-full border border-gray-300 rounded shadow-sm h-10 px-3 text-base" required>
                            <option value="" disabled {{ !old('entrada') ? 'selected' : '' }}>Seleccione el tipo de entrada</option>
                            <option value="Saldo Inicial" {{ old('entrada') == 'Saldo Inicial' ? 'selected' : '' }}>Saldo Inicial</option>
                            <option value="Insumos" {{ old('entrada') == 'Insumos' ? 'selected' : '' }}>Insumos</option>
                            <option value="Préstamo" {{ old('entrada') == 'Préstamo' ? 'selected' : '' }}>Préstamo</option>
                            <option value="Ajuste por transferencia" {{ old('entrada') == 'Ajuste por transferencia' ? 'selected' : '' }}>Ajuste por transferencia</option>
                            <option value="Ajuste de cierre" {{ old('entrada') == 'Ajuste de cierre' ? 'selected' : '' }}>Ajuste de cierre</option>
                            <option value="Tienda del peluquero" {{ old('entrada') == 'Tienda del peluquero' ? 'selected' : '' }}>Tienda del peluquero</option>
                            <option value="Otros" {{ old('entrada') == 'Otros' ? 'selected' : '' }}>Otros</option>
                        </select>
                    </td>
                </tr>
