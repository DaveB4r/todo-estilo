...
<div class="ml-4 flex items-center">
    <h1 class="text-3xl italic font-serif text-gray-800">Registrar Cuenta por Cobrar</h1>
</div>

<form action="{{ route('cuentas_por_cobrar.store') }}" method="POST" class="mt-6">
    @if ($errors->any())
    ...
    <table class="w-full space-y-4">
            ...
            <td>
                <select name="cliente_id" id="cliente_id" class="w-full border border-gray-300 rounded shadow-sm h-10 px-3 text-base" required>
                    ...
                    @foreach($clientes as $cliente)
                        <option value="{{ $cliente->identificacion }}">{{ $cliente->nombre }} {{ $cliente->apellido }}</option>
                    @endforeach
                </select>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td><input type="number" name="valor" id="valor" class="w-full border border-gray-300 rounded shadow-sm h-10 px-3 text-base"></td>
        </tr>
...
<script>
    document.getElementById('servicio_id').addEventListener('change', function () {
        ...
        if (precio) {
            // Reemplazar la coma decimal por nada (eliminar la parte decimal)
